...
public class DeviceServerController {
    ...
    public String deviceHome(Model model) {
        ...
        model.addAttribute("activeTab", "服务器");
    ...
    public String deviceDetail(@PathVariable("deviceID") Integer deviceID, Model model) {
        ...
        Device device = deviceServerService.getDeviceById(deviceID);
        if (device == null) {
            return "redirect:/device/home";
        }
    ...
    public String saveDevice(Model model, DeviceServer deviceServer, HttpServletRequest request) {
        List<Cabinet> cabinetList = cabinetService.findCabinetByName(deviceServer.getCabinet().getName());
        if (cabinetList != null && cabinetList.size() > 0) {
            deviceServer.setCabinetId(cabinetList.get(0).getId());
        }
        int result = deviceServerService.addDevcie(deviceServer, this.getLoginUser(request));
        if (result == 1) {
            model.addAttribute("result", "已添加1条记录!");
        }
        return "redirect:/device/server/list";
    }
    ...
    public String deleteDevice(Model model, @RequestParam(value = "deviceIDs[]") Long[] deviceIDs, HttpServletRequest request) {
        ...
        int result = deviceServerService.delDevice(deviceIDList, this.getLoginUser(request));
    ...
    public void expAllDevice2Excel(HttpServletRequest request,
        ...
        try {
            String fileName = "服务器设备台账.xls";
            byte[] bytes = deviceServerService.expAllDevice2Excel();
    ...
    public String findDeviceByDevice(Model model, DeviceServer deviceServer) {
        List<DeviceServer> deviceServerList = deviceServerService.findDeviceByDevice(deviceServer);
        List<DeviceNetwork> deviceNetworkList = deviceNetworkService.findAllDevice();
        List<DeviceOther> deviceOtherList = deviceOtherService.findAllDevice();
        List<DeviceSafety> deviceSafetyList = deviceSafetyService.findAllDevice();
        List<DeviceMultimedia> deviceMultimediaList = deviceMultimediaService.findAllDevice();
            ...
            if (!file.isEmpty()) {
                ...
                while (sheetIterator.hasNext()) {
                    ...
                    for (int i = 2; i <= lastRowNum; i++) {
                        ...
                        DeviceServer device = new DeviceServer();
                        HSSFCell cell;
                        int j = 0;
                        ...
                        try {
                            successRow += deviceServerService.addDevcie(device, this.getLoginUser(request));
                        }catch (Exception ex){
                            failureRow++;
                        }
