...
public class DeviceOtherController {
    ...
    public String deviceHome(Model model) {
        ...
        model.addAttribute("activeTab", "其他");
    ...
    public String deviceDetail(@PathVariable("deviceID") Integer deviceID, Model model) {
        ...
        Device device = deviceOtherService.getDeviceById(deviceID);
        if (device == null) {
            return "redirect:/device/home";
        }
    ...
    public String saveDevice(Model model, DeviceOther deviceOther, HttpServletRequest request) {
        ...
        return "redirect:/device/other/list";
    }
    ...
    public void expAllDevice2Excel(HttpServletRequest request,
        ...
        try {
            String fileName = "其他设备台账.xls";
            byte[] bytes = deviceOtherService.expAllDevice2Excel();
    ...
    public String findDeviceByDevice(Model model, DeviceOther deviceOther) {
        List<DeviceOther> deviceOtherList = deviceOtherService.findDeviceByDevice(deviceOther);
        List<DeviceNetwork> deviceNetworkList = deviceNetworkService.findAllDevice();
        List<DeviceMultimedia> deviceMultimediaList = deviceMultimediaService.findAllDevice();
        List<DeviceSafety> deviceSafetyList = deviceSafetyService.findAllDevice();
        List<DeviceServer> deviceServerList = deviceServerService.findAllDevice();
        ...
        model.addAttribute("deviceServerList", deviceServerList);
        model.addAttribute("deviceStorageList", deviceStorageList);
        model.addAttribute("deviceOther", deviceOther);
        model.addAttribute("roomList", roomList);
        model.addAttribute("activeTab", "其他");
            ...
            if (!file.isEmpty()) {
                ...
                while (sheetIterator.hasNext()) {
                    ...
                    for (int i = 2; i <= lastRowNum; i++) {
                        ...
                        DeviceOther device = new DeviceOther();
                        HSSFCell cell;
                        int j = 0;
                        ...
                        try {
                            successRow += deviceOtherService.addDevcie(device, this.getLoginUser(request));
                        }catch (Exception ex){
                            failureRow++;
                        }
