...
<div class="container-fluid" style="margin-top: 30px">
    <form id="do_form_query" class="form-inline">
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <div class="dropdown">
                        <label for="do_name">名称: </label>
                        <input type="text" class="form-control" id="do_name" name="name" value="<c:out value="${deviceOther.name}"/>" placeholder="请输入设备名称">
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="do_code">编号: </label>
                    <input type="text" class="form-control" id="do_code" name="code" value="<c:out value="${deviceOther.code}"/>" placeholder="请输入设备编号">
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="do_ipAddress"> IP: </label>
                    <input type="text" class="form-control" id="do_ipAddress" name="ipAddress" value="<c:out value="${deviceOther.ipAddress}"/>" placeholder="请输入IP地址">
                </div>
            </div>
        </div>
        <br>
        <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <div class="dropdown">
                        <label for="do_brand">品牌: </label>
                        <input type="text" class="form-control" id="do_brand" name="brand" value="<c:out value="${deviceOther.brand}"/>" placeholder="请输入设备品牌">
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="do_roomid">机房: </label>
                    <select id="do_roomid" name="roomId" class="form-control">
                        <option value="">全部</option>
                        <c:forEach var="room" items="${roomList}">
                            <option value="${room.id}" <c:if test="${deviceOther.roomId == room.id}">selected</c:if>>${room.name}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <button type="button" class="btn btn-primary" onclick="do_query()"> &nbsp;&nbsp;&nbsp;查&nbsp;&nbsp;&nbsp;询&nbsp;&nbsp;&nbsp;</button>
                </div>
            </div>
        </div>
    </form>
...
<div class="text-right" style="margin-bottom: 5px">
    ...
    <button class="btn btn-info" type="button" name="other" data-toggle="modal" data-target="#importModal">导入</button>
...
<script type="text/javascript">
    ...
    function do_exportExcel() {
        window.location.href = "/device/other/export";
    }

    function do_query(){
        var theForm = document.getElementById("do_form_query");
        theForm.action = "/device/other/query";
        theForm.method = "post";
        theForm.submit();
    }
    // 全选复选框,控制ID复选框
    ...
    function do_batchDelete() {
        ...
        if (IDs.length > 0) {
            ...
            $.post("/device/other/delete", indata, function (data) {
